...
const Navbar = () => {
    ...
    return (
       ...
       {navs.map(link => {
                    return <li><Link className='text-[25px] ' href={link == 'home' ? '/' : link}>{ link}</Link></li>
       })}
                <hr />
          ...
          <AiFillTwitterCircle className='text-slate-900 text-[20px] '/>
            ...
            <div  className='relative z-10 flex md:hidden items-center '>
              { showNav == 'right-[-1000px]' ? <AiOutlineMenu onClick={() => setShowNav('right-0')} className='text-[40px]' /> :
                <FaXmark onClick={() => setShowNav('right-[-1000px]')} className='text-[40px]'/>}
         </div>
       </nav>
    )
}
